...
StickerKitInfo::StickerKitInfo(const KeyValue &key)
    ...
    , m_materialPath{ key.GetString("sticker_material") }
...
static std::string SouvenirEventToken(std::string_view packageName)
{
    std::string token = Lowercase(packageName);
    ...
    if (token.rfind("csgo_", 0) == 0)
    {
        token.erase(0, 5);
    }

    if (token.rfind("crate_", 0) == 0)
    {
        token.erase(0, 6);
    }

    for (std::string_view mapPrefix : { "_de_", "_cs_", "_ar_" })
    {
        size_t mapPos = token.find(mapPrefix);
    ...
    constexpr std::string_view promoSuffix = "_promo";
    if (token.size() > promoSuffix.size() && token.compare(token.size() - promoSuffix.size(), promoSuffix.size(), promoSuffix) == 0)
    {
        token.erase(token.size() - promoSuffix.size());
    }

    return token;
}

static bool StringContainsAny(std::string_view value, const std::vector<std::string> &needles)
{
    std::string compactValue;
    compactValue.reserve(value.size());

    for (char c : value)
    {
        if (std::isalnum(static_cast<unsigned char>(c)))
        {
            compactValue.push_back(c);
        }
    }
    ...
    for (const std::string &needle : needles)
    {
        ...
        std::string compactNeedle;
        compactNeedle.reserve(needle.size());
        ...
        if (!compactNeedle.empty() && compactValue.find(compactNeedle) != std::string::npos)
        {
            return true;
        }
    }
...
static std::vector<std::string> SouvenirEventTokenAliases(std::string_view token)
{
    std::vector<std::string> aliases{ std::string{ token } };

    if (token == "esl14")
    {
        aliases.emplace_back("cologne2014");
        aliases.emplace_back("cologne_2014");
        aliases.emplace_back("eslcologne2014");
        aliases.emplace_back("esl_one_cologne_2014");
    }
    else if (token == "ems14")
    {
        aliases.emplace_back("katowice2014");
        aliases.emplace_back("katowice_2014");
        aliases.emplace_back("emskatowice2014");
        aliases.emplace_back("ems_one_katowice_2014");
    }

    return aliases;
}

const StickerKitInfo *ItemSchema::SouvenirEventStickerKitByPackageDefIndex(uint32_t packageDefIndex) const
{
    const ItemInfo *packageInfo = ItemInfoByDefIndex(packageDefIndex);
    if (!packageInfo)
    {
        return nullptr;
    }

    std::string token = SouvenirEventToken(packageInfo->m_name);
    if (token.empty())
    {
        return nullptr;
    }

    std::vector<std::string> aliases = SouvenirEventTokenAliases(token);

    for (const auto &pair : m_stickerKitInfo)
    {
        const StickerKitInfo &info = pair.second;
        if (info.m_tournamentEventId == 0 || info.m_tournamentTeamId != 0 || info.m_tournamentPlayerId != 0)
        {
            continue;
        }

        std::string search = Lowercase(info.m_name + " " + info.m_itemName + " " + info.m_materialPath);
        if (StringContainsAny(search, aliases))
        {
            return &info;
        }
    }
